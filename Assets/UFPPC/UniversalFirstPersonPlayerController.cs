...
namespace UFPPC
{
    ...
    public class UniversalFirstPersonPlayerController : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            {
                float energy = GameManager.Instance != null ? GameManager.Instance.playerEnergy : 100f;

                // im mniej energii, tym wiêksze wyg³adzenie (0.02 przy 100%, 0.55 przy 0%)
                float smoothTime = Mathf.Lerp(0.02f, 0.55f, 1f - (energy / 100f));

                // pobranie surowego ruchu myszy
                float mouseX = Input.GetAxis("Mouse X") * mouseSensitivity;
                float mouseY = Input.GetAxis("Mouse Y") * mouseSensitivity;

                // u¿yj SmoothDamp do wyg³adzenia
                Vector2 targetMouse = new Vector2(mouseX, mouseY);
                currentMouse = Vector2.SmoothDamp(currentMouse, targetMouse, ref mouseVelocity, smoothTime);
                ...
                transform.Rotate(Vector3.up * currentMouse.x);
            }
